...
{				
	...
	while (buffer != NULL)				
	{
		...
		{
			case 7: LOGE("SPS size:%d", size); m_sps.assign((char*)buffer,size); break;
			case 8: LOGE("PPS size:%d", size); m_pps.assign((char*)buffer,size); break;
			case 5: LOGE("IDR size:%d" ,size); 
