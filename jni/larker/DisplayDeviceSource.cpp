...
int  DisplayDeviceSource::Stats::notify(int tv_sec, int framesize)
{
	...
	if (tv_sec != m_fps_sec)
	{
		LOGE("m_msg:%ld, fps: %d, bandwidth: %d kbps",tv_sec, m_fps, m_size/128);
		m_fps_sec = tv_sec;
		m_fps = 0;
		m_size = 0;
	}
...
{			
	if (isCurrentlyAwaitingData()) 
	{
		...
		{				
			...
			LOGE("deliverFrame\ttimestamp:%ld.%ld,\tsize:%d,\tdiff:%d ms\t queue size:%d",
							 curTime.tv_sec  ,
							 curTime.tv_usec ,
							 fFrameSize , 
			...
			LOGE("PresentationTime:%ld",fPresentationTime);
...
void DisplayDeviceSource::pushRawData(char* d,unsigned int dataSize)
{
	...
	LOGE("DisplayDeviceSource::pushRawData");
...
int DisplayDeviceSource::getNextFrame() 
{
	...
	{
		// LOGE("rawDataQueue is empty");
	}
	else
	{
		LOGE("Queue pop front RawData");
		...
		LOGE("getNextFrame\ttimestamp:%ld.%ld\tsize:%d\tdiff:%d ms",
				ref.tv_sec,
				ref.tv_usec,
				frameSize,
				 (diff.tv_sec*1000+diff.tv_usec/1000)
...
void DisplayDeviceSource::processFrame(char * frame, int frameSize, const timeval &ref) 
{
	...
	while (!frameList.empty())
	{
		...
		LOGE("queueFrame\ttimestamp:%d.%d\tsize:%d\tdiff:%fms",
			ref.tv_sec,
			ref.tv_usec,
			size,
			diff.tv_sec*1000+diff.tv_usec/1000
...
void DisplayDeviceSource::queueFrame(char * frame, int frameSize, const timeval &tv) 
{
	...
	{
		LOGE("Queue full size drop frame size:%d",(int)m_captureQueue.size());		
...
{				
	...
	{
		LOGE("DisplayDeviceSource::splitFrames  frame empty");
	}
	return frameList;
}
